                ...
                <section class="content">
                    ...
                    <div class="box box-primary">
                        <div class="box-header with-border">
                            <h3 class="box-title">Data Pagu Global (Non Jurusan)</h3>
                        </div><br>
                        <!-- /.box-header -->
                        <div class="box-body">
                            <?php if ($this->session->userdata('akses') == 2) { ?>
                                <a href="<?= site_url('tambah-simulasi-nonjurusan'); ?>"> <button type="button" class="btn btn-primary"><i class="fa fa-plus"></i> Tambah Simulasi</button><br><br></a>
                            <?php } ?>
                            ...
                            <table id="dataKriteria" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php foreach ($simulasi_jurusan as $key => $value) {
                                        ...
                                        <td>
                                            <?php if ($this->session->userdata('akses') == 2) { ?>
                                                <?php if ($value->isApproval == 0) { ?>
                                                    <a class="btn btn-success btn-sm" href="<?= base_url('edit-simulasi-nonjurusan/' . $value->id_simulasi) ?>"><i class="fa fa-edit"></i> Edit</a>
                                                    <a class="btn btn-info btn-sm" href="<?= base_url('hasil-simulasi-nonjurusan/' . $value->id_simulasi) ?>"><i class="fa fa-eye"></i> Cek Data</a>
                                                    <a class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin akan menghapus data simulasi ini.?')" href="<?= base_url('Pagu_nonjurusan/hapusData/' . $value->id_simulasi) ?>"><i class="fa fa-trash"></i> Hapus</a>
                                        ...
                                        <!-- <td>
                                                <a class="btn btn-success btn-sm" href="<?= base_url('edit-simulasi-nonjurusan/' . $value->id_simulasi) ?>"><i class="fa fa-edit"></i> Edit</a>
                                                <a class="btn btn-info btn-sm" href="<?= base_url('hasil-simulasi-nonjurusan/' . $value->id_simulasi) ?>"><i class="fa fa-eye"></i> Lihat</a>
                                                <a class="btn btn-warning btn-sm" href="<?= base_url('Pagu_nonjurusan/perbaruiData/' . $value->id_simulasi) ?>"><i class="fa fa-refresh"></i> Perbarui</a>
                                                <a class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin akan menghapus data simulasi ini.?')" href="<?= base_url('Pagu_nonjurusan/hapusData/' . $value->id_simulasi) ?>"><i class="fa fa-trash"></i> Hapus</a>
